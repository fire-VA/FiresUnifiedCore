...
{
    /// <summary>
    /// Compatibility shim that replicates the UI-facing behaviour of
    /// AzuExtendedPlayerInventory when the mod is no longer installed.
    ///
    /// Manages visibility, button wiring, player preview camera, and panel toggling
    /// for AzuEPI-injected elements. Does NOT attempt to manage inventory grid slots
    /// or equipment slot wiring — those are complex systems that require the actual
    /// mod's Harmony patches to function correctly.
    ///
    /// Attached as a MonoBehaviour by <see cref="UIVanillaOverrideManager"/> when it
    /// detects that AzuEPI was a dependency of the active override layout AND is no
    /// longer loaded.
    /// </summary>
    public class UIOverrideAzuEPICompat : MonoBehaviour
    {
        ...
        private void UpdatePlayerPreviewCamera(Player player)
        {
            ...
            float radY = _previewRotationY * Mathf.Deg2Rad;
            Vector3 offset = new Vector3(
                Mathf.Sin(radY) * _previewDistance,
                0.4f,
                Mathf.Cos(radY) * _previewDistance
            );
        ...
        public static bool TryAttach(UILayoutDefinition layout, GameObject vanillaRoot)
        {
            ...
            if (!string.IsNullOrEmpty(extraRowsMeta) && int.TryParse(extraRowsMeta, out int er))
                compat.ExtraRows = er;

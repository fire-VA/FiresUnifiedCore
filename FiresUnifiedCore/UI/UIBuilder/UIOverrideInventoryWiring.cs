...
{
    /// <summary>
    /// Orchestrates wiring of inventory slot/hotbar mirrors onto injected override
    /// elements based on their <see cref="UIOverrideElementTags"/> tags and per-element
    /// metadata (slot_id, grid_x, grid_y, hotbar_index).
    ///
    /// Called by <see cref="UIVanillaOverrideManager"/> during WireInjectedElements
    /// after the override layout is instantiated onto the vanilla hierarchy.
    ///
    /// <b>Design principle:</b> This class is the ONLY place that attaches
    /// UIOverrideSlotMirror, UIOverrideSlotInteraction, and UIOverrideHotbarSlotMirror
    /// components. All other override code stays out of inventory management.
    ///
    /// The wiring uses element metadata set in the UI Builder editor:
    ///   - "slot_id"       ? Slots.Slot ID (e.g., "Helmet", "Food", "ToolHotbar1")
    ///   - "grid_x"        ? Grid position X (fallback when slot_id is absent)
    ///   - "grid_y"        ? Grid position Y
    ///   - "hotbar_index"  ? Tool hotbar index (0-based), for hotbar elements
    ///   - "interactive"   ? "true" to enable click/drag forwarding
    /// </summary>
